...
export const them_nha_sx = async () => {
    ...
    let tt:string =  (document.querySelector("#thu_tu") as HTMLInputElement).value;
    let ah:string = (document.querySelector('[name=an_hien]:checked') as HTMLInputElement).value;


    let data = { ten:ten, thu_tu:tt, an_hien:ah };
    let opt = { method: 'post', body:JSON.stringify(data), headers:{'Content-type': 'application/json'} }
    let kq = await fetch(urlserver + "/nha_sx", opt).then(res => res.json()).then ( d=> d)
