    ...
    return `
     <form class='col-10 m-auto border border-primary p-2'>
        ...
        <div class='mb-3 d-flex'>
            ...
            <div class='col-6'>
                Màu sắc <select id='mau_sac' class='form-control border-primary' >${mausac_option}</select>            </div>
        </div>
        <div class='mb-3 d-flex'>
            <div class='col-6'>
...
export const them_san_pham = async () =>{
  ...
  let hot:string = (document.querySelector("input[name='hot']:checked") as HTMLInputElement).value;
  let data = {
    ten: ten, ngay: ngay, gia: gia, gia_km: gia_km, hinh: hinh, xem: xem, 
    mau_sac: mau_sac, id_nha_sx: id_nha_sx, can_nang: can_nang, an_hien: an_hien, hot: hot
  }
...
export const list_san_pham = async (sosp:number)=> {
  ...
  <div id='data'>
     <div class='sp'>
     <b>Hình</b> <b>ID, Tên, Ngày</b> <b>Giá & Trạng thái</b> <b>Thông tin</b> <b>Thao tác</b>
     </div>
  ${str}</div>
    </div>`;
  return str;
...
<div class='sp'>
    ...
    <span> Màu sắc : ${sp.mau_sac} <br> Cân nặng : ${sp.can_nang} kq <br>
           Lượt xem: ${sp.xem} lượt<br> Tính chất: ${keyCuaTinhChat(sp.tinh_chat)}
    </span>
    <span>
  ...
  for (let key in TINH_CHAT){
    ...
    return `
     <form class='col-10 m-auto border border-primary p-2'>
        <div class='mb-3 d-flex'>
            <div class='col-6'>
                Tên SP <input id='ten' value='${sp.ten}' class='form-control border-primary' type='text'>
            </div>
            <div class='col-6'> 
                Ngày <input id='ngay' value='${sp.ngay}' class='form-control border-primary' type='date'>
            </div>
        </div>
        <div class='mb-3 d-flex'>
            <div class='col-6'>
                Giá gốc <input id='gia'  value='${sp.gia}' class='form-control border-primary' type='number'>
            </div>
            <div class='col-6'>
                Giá KM <input id='gia_km'  value='${sp.gia_km}' class='form-control border-primary' type='number'>
            </div>
        </div>
        <div class='mb-3 d-flex'>
            <div class='col-6'>
                Hình <input id='hinh'  value='${sp.hinh}' class='form-control border-primary' type='text'>
            </div>
            <div class='col-6'>
                Lượt xem <input id='xem'  value='${sp.xem}' class='form-control border-primary' type='number'>
            </div>
        </div>
        <div class='mb-3 d-flex'>
            <div class='col-6'>
        ...
        <div class='mb-3 d-flex'>
            ...
            <div class='col-6'>
                Cân nặng <input id='can_nang'  value='${sp.can_nang}' class='form-control border-primary' type='text'>
            </div>
        </div>
        <div class='mb-3 d-flex'>
            <div class='col-6'>
                ...
                <input name='an_hien' value='1' ${sp.an_hien=='1'? 'checked':''} type='radio' > Hiện
            </div>
            <div class='col-6'>
                Nổi bật
                <input name='hot' value='0'  ${sp.hot=='0'? 'checked':''} type='radio' > Bình thường
...
export const sua_san_pham = async () =>{
  ...
  let hot:string = (document.querySelector("input[name='hot']:checked") as HTMLInputElement).value;
  let data = {
    ten: ten, ngay: ngay, gia: gia, gia_km: gia_km, hinh: hinh, xem: xem, tinh_chat:tinh_chat, id:id,
    mau_sac: mau_sac, id_nha_sx: id_nha_sx, can_nang: can_nang, an_hien: an_hien, hot: hot
  }
